...
const Customize_image = ({
  ...
  return (
    <div className="w-full flex justify-center">
      <div
        ref={containerRef}
        className="relative w-full max-w-4xl h-[420px] sm:h-[480px] md:h-[560px] overflow-hidden rounded-xl bg-slate-50"
      ...
      >
        <img
          ...
          className="block w-full h-full object-cover rounded-xl select-none pointer-events-none"
          draggable={false}
        />
        ...
        {renderItems.map((item) => {
          if (item.type === "text") {
            ...
            // If container dimensions are available, render a react-rnd wrapper for better resizing UX
            const cw = containerDimensions.width || 0;
            const ch = containerDimensions.height || 0;

            // Calculate pixel values
            const widthPx = cw ? (item.width / 100) * cw : 100;
            const heightPx = ch ? (item.height / 100) * ch : 100;
            const xPx = cw ? (item.relativeX / 100) * cw - widthPx / 2 : 0;
            const yPx = ch ? (item.relativeY / 100) * ch - heightPx / 2 : 0;
            ...
            return (
              <React.Fragment key={item.id}>
                {cw > 0 && ch > 0 ? (
                  <Rnd
                    bounds="parent"
                    size={{ width: Math.max(20, widthPx), height: Math.max(20, heightPx) }}
                    position={{ x: Math.max(0, xPx), y: Math.max(0, yPx) }}
                    onDragStart={() => onSelectElement && onSelectElement(item.id)}
                    onDragStop={(e, d) => {
                      const newRelX = ((d.x + (e.currentTarget?.offsetWidth || widthPx) / 2) / cw) * 100;
                      const newRelY = ((d.y + (e.currentTarget?.offsetHeight || heightPx) / 2) / ch) * 100;
                      if (onMoveImage) onMoveImage(item.id, Math.max(0, Math.min(100, newRelX)), Math.max(0, Math.min(100, newRelY)));
                    }}
                    onResizeStop={(e, direction, ref, delta, position) => {
                      const newW = ref.offsetWidth;
                      const newH = ref.offsetHeight;
                      const newRelW = (newW / cw) * 100;
                      const newRelH = (newH / ch) * 100;
                      const newRelX = ((position.x + newW / 2) / cw) * 100;
                      const newRelY = ((position.y + newH / 2) / ch) * 100;
                      if (onResizeImage) onResizeImage(item.id, { width: newRelW, height: newRelH });
                      if (onMoveImage) onMoveImage(item.id, Math.max(0, Math.min(100, newRelX)), Math.max(0, Math.min(100, newRelY)));
                    }}
                  ...
                  >
                    <div
                      onClick={(e) => {
                        e.stopPropagation();
                        if (onSelectElement) onSelectElement(item.id);
                      }}
                      style={{ width: '100%', height: '100%', position: 'relative' }}
                    >
                      <img src={item.url} alt="Uploaded" className="w-full h-full object-contain" draggable={false} />

                      {selectedElementId === item.id && (
                        <>
                          <Popover
                            open={showToolbarFor === item.id}
                            onOpenChange={(open) => setShowToolbarFor(open ? item.id : null)}
                          >
                            <PopoverTrigger asChild>
                              <button
                                type="button"
                            ...
                            <PopoverContent side="right" align="start" className="p-0">
                              <ZIndexToolbar
                                onBringForward={() => onBringForward && onBringForward(item.id)}
                                onBringToFront={() => onBringToFront && onBringToFront(item.id)}
                                onSendBackward={() => onSendBackward && onSendBackward(item.id)}
                                onSendToBack={() => onSendToBack && onSendToBack(item.id)}
                                showLayers={showLayers}
                                setShowLayers={setShowLayers}
                                {...getZOrderState(item, elements, texts, images)}
                              />
                  ...
                  // Fallback for zero-dimensions: render static img
                  <div key={item.id} style={{ position: 'absolute', left: `${item.relativeX}%`, top: `${item.relativeY}%`, transform: 'translate(-50%,-50%)' }}>
                    <img src={item.url} alt="Uploaded" className="w-24 h-24 object-contain" draggable={false} />
                  </div>
                )}
              </React.Fragment>
            );
